...
def enviar_mensaje_twilio_orig(telefono, texto, max_retries=3):
    ...
    for intento in range(max_retries):
        try:
            print(
                f"📤 Enviando mensaje con Twilio (intento {intento + 1}/{max_retries}): {texto[:50]}..."
            )
...
def enviar_mensaje_twilio(telefono, texto, max_retries=3):
    ...
    too_long_ending_text = (
        "La respuesta del asistente es demasiado larga, ha sido recortada"
    )
...
def procesar_mensajes_entrantes_funcional(form_data, redis_client=r):
    ...
    if body and "borrar memoria" in body.lower():
        ...
        resultado = enviar_mensaje_twilio(from_number, "✅ Tu memoria ha sido borrada.")
        if resultado.get("success", False):
            redis_client.set(dedup_key, "exists", ex=600)
            print(f"✅ Borrar memoria procesado: {dedup_key}")
        return resultado

    # ✅ Procesa mensajes de audio (con Celery y ACK inmediato)
    if message_type == "audio":
        # Si feature está deshabilitada, solo guardar como referencia
        if not VOICE_NOTES_ENABLED:
            print(f"🔇 VOICE_NOTES_ENABLED=false, ignorando audio para: {phone_number}")
            enviar_mensaje_twilio(
                from_number,
                "📝 Las notas de voz aún no están habilitadas. Por favor, envía un mensaje de texto.",
            )
        ...
        # 1️⃣ ENVIAR ACK INMEDIATO
        print(f"📨 Enviando ACK inmediato al usuario...")
        ack_result = enviar_mensaje_twilio(
            from_number,
            "🎙️ Recibí tu nota de voz. La estoy transcribiendo, dame unos segundos...",
        )

        # Si no se puede enviar el ACK, abortar
        if not ack_result.get("success", False):
            print(f"⚠️ No se pudo enviar ACK: {ack_result.get('error')}")
            return "ACKFailed"

        # 2️⃣ PREPARAR JOB PARA CELERY
        ...
        # 3️⃣ ENCOLAR EN CELERY (+ Redis fallback)
        try:
            ...
            print(f"✅ Audio encolado a Celery: {result.id}")
        except Exception as e:
            print(f"⚠️ Error enqueueing to Celery: {e}, usando Redis fallback")
    ...
    if message_type == "document":
        ...
        enviar_mensaje_twilio(
            from_number, f"📄 Subiendo '{nombre_base}.{extension}' a Google Drive..."
        )

        # Descargar archivo de Twilio
        ruta_temporal = descargar_documento_de_twilio(
            media_url=media_url, nombre_archivo=nombre_base, tipo_archivo=extension
        )

        if not ruta_temporal:
            enviar_mensaje_twilio(
                from_number, "❌ Lo siento, hubo un error al descargar tu documento."
            )
            redis_client.set(dedup_key, "exists", ex=600)
        ...
        if resultado and resultado.get("success"):
            mensaje = (
                f"✅ Documento '{nombre_base}.{extension}' subido exitosamente!\n\n"
            )
            ...
            error_msg = (
                resultado.get("error", "Desconocido") if resultado else "Desconocido"
            )
            enviar_mensaje_twilio(
                from_number, f"❌ Error al subir a Drive: {error_msg}"
            )
    ...
    try:
        print(
            "DICT CONVERSATION ASSISTANT SUPABASE", dict_conversation_assistant_supabase
        )
    except:
        print("ERROR AL IMPRIMIR DICT CONVERSATION ASSISTANT SUPABASE")

    # Envía respuesta al usuario con Twilio
    resultado_envio = enviar_mensaje_twilio(from_number, str(answer_data["answer"]))

    # 🔥 CRÍTICO: Solo completar si el envío fue exitoso
    if not resultado_envio.get("success", False):
        print(
            f"⚠️ ADVERTENCIA: Mensaje no enviado. Error: {resultado_envio.get('error', 'Desconocido')}"
        )
        return "ErrorEnvio"
    ...
    add_to_chat_history(
        id_chat_history, answer_data["answer"], "assistant", phone_number
    )
...
def procesar_mensajes_entrantes(form_data, redis_client=r):
    ...
    if len(answer_text) > MAX_LENGTH:
        ...
        print(
            f"⚠️ Respuesta truncada de {len(answer_data['answer'])} a {MAX_LENGTH} caracteres"
        )
