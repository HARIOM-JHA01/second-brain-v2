...
from agente_rolplay.config import ANALYTICS_LOG_DIR as LOG_DIR, ANALYTICS_LOG_FILE as LOG_FILE
...
def log_greeting(phone_number: str, language: str = "es"):
    ...
    from agente_rolplay.messaging.greeting_handler import get_intro_message
...
def log_help(phone_number: str, language: str = "es"):
    ...
    from agente_rolplay.messaging.greeting_handler import get_capabilities_message
...
def log_message_to_db(
    ...
    """
    Write a MessageLog record to PostgreSQL in a background thread.
    Silently skips on any error so it never breaks the main message flow.
    """

    def _write():
        try:
            from agente_rolplay.db.database import get_db
            from agente_rolplay.db.models import MessageLog, Profile
            ...
            print(f"Warning: could not write MessageLog: {e}")
