...
def obtener_servicio():
    ...
    CREDENTIALS_PATH = os.path.join(os.path.dirname(__file__), DRIVE_CREDENTIALS)
    TOKENS_PATH = os.path.join(os.path.dirname(__file__), DRIVE_TOKENS)
...
def buscar_carpeta_por_nombre(nombre_carpeta):
    ...
    try:
        print(f"Searching folder: '{nombre_carpeta}'...")

        # Query to search for folders with specific name
        ...
        if not folders:
            print(f'No folder found with name "{nombre_carpeta}"')
            return []
...
def listar_archivos_en_carpeta(nombre_carpeta=None, folder_id=None):
    ...
    Args:
        ...
        folder_id (str): Folder ID (more precise)
    ...
    try:
        # If we don't have folder_id, search by name
        if not folder_id and nombre_carpeta:
            folders = buscar_carpeta_por_nombre(nombre_carpeta)
            if not folders:
                return []
            folder_id = folders[0]["id"]
            print(f"\nUsing folder: {folders[0]['name']} (ID: {folder_id})")

        if not folder_id:
            print("You must provide nombre_carpeta or folder_id")
            return []

        print(f"\nListing files in folder...")

        # Query to get files within the folder
...
def obtener_id_carpeta_por_nombre(nombre_carpeta):
    """
    Gets folder ID from folder name

    Args:
        nombre_carpeta (str): Folder name
...
def subir_archivo_a_drive(ruta_archivo_local, nombre_carpeta=None, folder_id=None):
    ...
    Args:
        ...
        nombre_carpeta (str, optional): Folder name in Drive
        folder_id (str, optional): Folder ID (more precise)

    Returns:
        dict: Information about uploaded file (id, name, webViewLink)
    """
    service = obtener_servicio()
    ...
    if not os.path.exists(ruta_archivo_local):
        print(f"File does not exist: {ruta_archivo_local}")
        return None
    ...
    try:
        ...
        # If we have folder name but no ID, search for it
        if nombre_carpeta and not folder_id:
            folders = buscar_carpetas_drive(nombre_carpeta=nombre_carpeta)
            if folders:
                folder_id = folders[0]["id"]
                print(f"Using folder: {folders[0]['name']} (ID: {folder_id})")
            else:
                print(f"Folder '{nombre_carpeta}' not found. Uploading to root.")
        ...
        print(f"Uploading file: {nombre_archivo}")
        print(f"   Type: {mime_type}")
        print(f"   Size: {os.path.getsize(ruta_archivo_local) / 1024:.2f} KB")

        # File metadata
        file_metadata = {"name": nombre_archivo}

        # If we have folder_id, add it
        if folder_id:
...
def procesar_todos_los_documentos(archivos_json="estructura_second_brain.json"):
    ...
    tipos_soportados = ["application/pdf", "application/vnd.google-apps.document"]

    archivos_a_procesar = [
        archivo for archivo in archivos if archivo.get("mimeType") in tipos_soportados
    ]

    print(f"📚 Archivos a procesar: {len(archivos_a_procesar)}")
    print(
        f"   PDFs: {sum(1 for a in archivos_a_procesar if a['mimeType'] == 'application/pdf')}"
    )
    print(
        f"   Google Docs: {sum(1 for a in archivos_a_procesar if 'document' in a['mimeType'])}"
    )
    ...
    for i, archivo in enumerate(archivos_a_procesar, 1):
        ...
        try:
            texto = extraer_texto_de_archivo(archivo["id"], archivo["mimeType"])
            ...
            if texto:
                ...
                print(
                    f"✅ Texto extraído: {num_caracteres} caracteres, {num_palabras} palabras"
                )
...
def guardar_textos_extraidos(resultados, nombre_archivo="textos_extraidos.json"):
    ...
    Args:
        resultados (List[Dict]): List with extracted texts
        nombre_archivo (str): Output filename
    """
    print(f"\nSaving extracted texts to: {nombre_archivo}")

    with open(nombre_archivo, "w", encoding="utf-8") as f:
        json.dump(resultados, f, indent=2, ensure_ascii=False)

    # Calculate file size
    size_mb = os.path.getsize(nombre_archivo) / (1024 * 1024)

    print(f"File saved: {size_mb:.2f} MB")

    # Summary
    total_chars = sum(r["num_caracteres"] for r in resultados)
    total_words = sum(r["num_palabras"] for r in resultados)

    print(f"\nSUMMARY:")
    print(f"   Total documents: {len(resultados)}")
    print(f"   Total characters: {total_chars:,}")
    print(f"   Total words: {total_words:,}")


def get_drive_structure_and_extract_text(
    extracted_texts_path="textos_drive_extraidos.json",
    drive_structure="estructura_second_brain.json",
    ...
    archivos_con_rutas = guardar_estructura_en_json(
        second_brain_folder_id, drive_structure
    )
    ...
    print(
        "Se ha concluido con el proceso de obtener toda la estructura del Drive del 2nd Brain."
    )
...
if __name__ == "__main__":
    ...
    # process to get complete structure and save to JSON
    # second_brain_folder_id = "1ciaGOxXKcLMNeuIa9103ivxYgBon__7q"

    # # Option 1: Get all IDs only
    # print("\n" + "=" * 80)
    ...
    # print("\n" + "=" * 80)
    # print(f"TOTAL FILES FOUND: {len(todos_archivos)}")
    # print("=" * 80)
    ...
    # # Option 2: With full paths and save to JSON
    # print("\n" + "=" * 80)
    # print("SAVING COMPLETE STRUCTURE")
    # print("=" * 80)
    ...
    # # Test 2: Search for specific folder
    # print("\nTest 2: Search for 'Documentos' folder")
    # # carpetas_docs = buscar_carpeta_por_nombre("Second Brain")
    # carpetas = buscar_carpetas_drive(nombre_carpeta="Second Brain")
    # breakpoint()

    # Test 3: List files in a folder
    # if carpetas:
    #     print("\nTest 3: List files in first folder")
