...
def chunk_text_for_embedding(
    text: str, max_tokens: int = 7000, overlap_tokens: int = 400
) -> list[str]:
    """
...
def upload_to_pinecone(
    ...
    try:
        ...
        chunks = chunk_text_for_embedding(text=text, max_tokens=7000, overlap_tokens=400)
        if not chunks:
        ...
        for chunk_index, chunk_text in enumerate(chunks):
            text_preview = chunk_text[:1000] if len(chunk_text) > 1000 else chunk_text
